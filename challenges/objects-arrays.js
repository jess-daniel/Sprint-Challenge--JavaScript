...
const dino1 = {
  ...
  roar: function() {console.log("RAWERSRARARWERSARARARRRR!")}
}
...
let universities = [];
universities = graduates.map(uni => uni.university);
universities.sort();
...
let contactInfo = [];
contactInfo = graduates.map(student => `${student.first_name} ${student.email}`);
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
let unisWithUni = [];
unisWithUni = graduates.filter(student => student.university.includes("Uni"));
...
lowCaseAnimalNames = zooAnimals.map(animal => animal.animal_name.toLocaleLowerCase());
...
populationArray = [];
populationArray = zooAnimals.map(animal => animal.population);
// console.log(populationArray);
populationTotal = populationArray.reduce((total, i) => {
  return total + i;
}, 0)
